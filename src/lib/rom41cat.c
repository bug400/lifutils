...
unsigned int rom41cat_read_rom(FILE *fp,unsigned int* rom)
/* Read a ROM image from fp */
  {
    ...
    while(fread(word, sizeof(unsigned char), 2, fp)==2)
      {
...
unsigned char rom41cat_disp2asc (unsigned char disp_char)
...
void rom41cat_display_names(unsigned int * rom, unsigned int rom_size, int xrom_flag, int utf_flag)
        ...
        for(func=0; func<n_funcs; func++)
          {
            ...
            if(focal_flag) {
               ...
               addr= start_address;
               do {
                  c=rom41cat_disp2asc((rom[--addr]&0x7f));
                  fname[l]=((c==' ')?'_':c);
                  l++;
               }
               while(!(rom[addr]&0x80));
...
void rom41cat_usage(void)
  {
    fprintf(stderr,"Usage : lifutils rom41cat [-x] < input file [> output file]\n");
    fprintf(stderr,"        or\n");
    fprintf(stderr,"        lifutils rom41cat [-x] INPUTFILE [> output file]\n");
    fprintf(stderr,"        Display the function names in a HP41 ROM image\n");
...
int rom41cat(int argc, char **argv)
  {
    ...
    /* Read in the ROM */
    rom_size=rom41cat_read_rom(fp,rom);
    /* And print the function names */
    rom41cat_display_names(rom, rom_size,xrom_flag, utf_flag);
